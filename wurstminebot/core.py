...
def update_topic(force=None, special_status=object()):
    ...
    if usc_config.get('nextDate') is not None:
        ...
        topic_parts.append('{0} on {1:%Y-%m-%d} at {1:%H:%M} UTC{2}'.format('Next USC' if usc_config.get('completedSeasons', usc_config) is None else 'USC {}'.format(usc_config['completedSeasons'] + 1), next_usc, '' if next_usc_local.replace(tzinfo=None) == next_usc.replace(tzinfo=None) else ' ({}{:%H:%M} local time)'.format('' if next_usc_local.date == next_usc.date else '{:%Y-%m-%d} '.format(mext_usc_local), next_usc_local)))
