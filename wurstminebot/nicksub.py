...
def config(person_id=None):
    ...
    for person in j:
        ...
        raise PersonNotFoundError(
            'person with id ' + str(person_id) + ' not found')
...
class PersonNotFoundError(Exception):
    # raised when a Person object is created or reloaded with data not in the
    # config
    pass
...
class BasePerson:
    ...
    def option(self, option_name):
        # These options are on by default. All other options are off by
        # default.
    ...
    def __init__(self, id_or_nick, context=None, strict=False):
        ...
        if context is None:
            ...
            for id, irc_nick in ircNicks(mode='all', include_ids=True):
                ...
                raise PersonNotFoundError(
                    'person with IRC nick ' + str(id_or_nick) + ' not found')
        elif context == 'minecraft':
            ...
            for id, reddit_nick in redditNicks(include_ids=True, format='plain'):
                ...
                raise PersonNotFoundError(
                    'person with reddit nick ' + str(id_or_nick) + ' not found')
        elif context == 'twitter':
            for id, twitter_nick in twitterNicks(include_ids=True, twitter_at_prefix=False):
                ...
                raise PersonNotFoundError(
                    'person with twitter nick ' + str(id_or_nick) + ' not found')
        else:
...
def sub(nick, source, target, strict=True, exit_on_fail=False, twitter_at_prefix=True):
    if exit_on_fail:
        if nick is None:
            sys.exit(1)
        try:
            ret = Person(nick, source).nick(
                target, twitter_at_prefix=twitter_at_prefix)
            if ret is None:
                sys.exit(2)
            else:
                return ret
...
def textsub(text, source, target, strict=False):
    ...
    for id, nick in _nicksForContext(source):
        if Person(id).nick(target):
            text = re.sub('(?<!' + urlcharacters + ')(' + re.sub('\\|', '\\|', nick) + ')(?!' + urlcharacters + ')', Person(
                id).nick(target, twitter_at_prefix=True), text, flags=re.IGNORECASE)
    if not strict:
